...
BUZZER = 35  # Physical pin 35 for Buzzer
    ...
    while True:
        ...
        if buzzer_active:
            if current_time - last_beep_time >= 0.5:
                # Toggle buzzer
                GPIO.output(BUZZER, not GPIO.input(BUZZER))
                last_beep_time = current_time
                beep_count += 1
                if beep_count >= 6:  # 3 full cycles (on/off), 6 toggles
                    GPIO.output(BUZZER, False)
                    buzzer_active = False
                    beep_count = 0
        ...
        if state == WAIT_FOR_GEAR:
            ...
            if elapsed < 5:
                ...
                if distance > 6 and not area_cleared:
                    area_cleared = True
                if not area_cleared:
                    cv2.putText(frame, "Ejecting gear...", (10, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                    ...
                    cv2.putText(frame, f"Distance: {distance}cm", (10, 60),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                if area_cleared and 0 < distance <= 6:
                    GPIO.output(MOTOR, False)
                    state = WAIT_FOR_STABILIZE
                    stabilize_start_time = time.time()
                    print("\nGear detected. Waiting 3 seconds to stabilize...")
